...
namespace iTuner
{
	...
	public class UsbManager : IDisposable
	{
		...
		public UsbDiskCollection GetAvailableDisks ()
		{
            ...
            foreach (ManagementObject drive in _searcher.Get())
            //foreach (ManagementObject drive in
            //    new ManagementObjectSearcher(
            //        "select DeviceID, Model from Win32_DiskDrive ").Get())
			{
                ...
                string _AssocQuery = "Associators of {Win32_DiskDrive.DeviceID='" +
                                          drive.Properties["DeviceID"].Value.ToString() + "'}" +
    ...
    
				{
                    ...
                    foreach (var _LogDiskEnu in _LogDisk.Get())
					{
                        ...
                        UsbDisk disk = new UsbDisk(_objAssocPart["Name"].ToString());
                        disk.Model = drive["Model"].ToString();
                        disk.Volume = _LogDiskEnu["VolumeName"].ToString();
                        disk.DeviceId = _LogDiskEnu["DeviceId"].ToString();
                        disk.FreeSpace = (ulong)_LogDiskEnu["FreeSpace"];
                        disk.Size = (ulong)_LogDiskEnu["Size"];

                      disk.Description=  _LogDiskEnu.Properties["Description"].Value.ToString();
                      disk.DriveType = _LogDiskEnu.Properties["DriveType"].Value.ToString();
                      disk.MediaType = _LogDiskEnu.Properties["MediaType"].Value.ToString();
